...
{
    public class DebugDoorMiniGameSystem: ReactiveSystem<GameEntity>
    {
        private readonly IGroup<GameEntity> _playerInventories;
        private readonly IGroup<GameEntity> _worldInventories;

        public DebugDoorMiniGameSystem(Contexts contexts) : base(contexts.game)
        {
        ...
        {
            return entity.isTriggered && entity.isDebugDoorMiniGameActivator;
        }
        ...
        protected override void Execute(List<GameEntity> items)
        {
            ...
            {
                if (playerInventory.PlayerItemList.Contains(ItemID.DebugKey))
                {
                    playerInventory.PlayerItemList.Remove(ItemID.DebugKey);
                    worldInventory.WorldItemList.Remove(item.ItemID);
                }
                else item.isTriggered = false;
            }
        }
    }
}